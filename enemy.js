class Enemy {
  constructor(canvas, x, y) {
    this.randomNumber = function randomInt(min, max) {
  ...
  drawImg() {
    if (!this.imageSelected && Math.random() > 0.7) {
      this.enemyFigure = true;
    }
    if (!this.imageSelected && Math.random() > 0.5) {
      this.healClick = true;
    }

    if (!this.imageSelected && !this.enemyFigure) {
      this.imageSelected = this.icons[
        Math.floor(Math.random() * this.icons.length)
      ];
    } else if (!this.imageSelected && this.enemyFigure) {
      this.imageSelected = this.iconsFail[
        Math.floor(Math.random() * this.iconsFail.length)
      ];
    }
  ...
  checkPairClick(userX, userY) {
    if (
      userX > this.x &&
      userX < this.x + this.size &&
      userY > this.y &&
      userY < this.y + this.size
    ) {
      return true;
    } else {
      return false;
